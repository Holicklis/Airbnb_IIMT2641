...
correlations <- data.frame(
  Accuracy = cor(filtered_data$review_scores_accuracy, filtered_data$review_scores_rating, use = "complete.obs"),
  Cleanliness = cor(filtered_data$review_scores_cleanliness, filtered_data$review_scores_rating, use = "complete.obs"),
  Checkin = cor(filtered_data$review_scores_checkin, filtered_data$review_scores_rating, use = "complete.obs"),
  Communication = cor(filtered_data$review_scores_communication, filtered_data$review_scores_rating, use = "complete.obs"),
  Location = cor(filtered_data$review_scores_location, filtered_data$review_scores_rating, use = "complete.obs"),
  Value = cor(filtered_data$review_scores_value, filtered_data$review_scores_rating, use = "complete.obs")
)
# accuracy
p_acc <- ggplot(filtered_data, aes(x = review_scores_accuracy, y = review_scores_rating, color=price)) + 
  geom_jitter(size = 0.4, alpha = 0.2, na.rm = TRUE) +
  geom_text(data = correlations, y = 0.2, aes(x = 4, label = paste("Correlation = ", Accuracy)), color = "black", size = 3) +
  scale_color_continuous(name = "Price")

# cleanliness
p_clean <- ggplot(filtered_data, aes(x = review_scores_cleanliness, y = review_scores_rating, color=price)) + 
  geom_jitter(size = 0.4, alpha = 0.2, na.rm = TRUE) +
  geom_text(data = correlations, y = 0.2, aes(x = 4, label = paste("Correlation = ", Accuracy)), color = "black", size = 3) +
  scale_color_continuous(name = "Price")

# checkin
p_check <- ggplot(filtered_data, aes(x = review_scores_checkin, y = review_scores_rating, color=price)) + 
  geom_jitter(size = 0.4, alpha = 0.2, na.rm = TRUE) +
  geom_text(data = correlations, y = 0.2, aes(x = 4, label = paste("Correlation = ", Accuracy)), color = "black", size = 3) +
  scale_color_continuous(name = "Price")

# communication
p_com <- ggplot(filtered_data, aes(x = review_scores_communication, y = review_scores_rating, color=price)) + 
  geom_jitter(size = 0.4, alpha = 0.2, na.rm = TRUE) +
  geom_text(data = correlations, y = 0.2, aes(x = 4, label = paste("Correlation = ", Accuracy)), color = "black", size = 3) +
  scale_color_continuous(name = "Price")

# location
p_loc <- ggplot(filtered_data, aes(x = review_scores_location, y = review_scores_rating, color=price)) + 
  geom_jitter(size = 0.4, alpha = 0.2, na.rm = TRUE) +
  geom_text(data = correlations, y = 0.2, aes(x = 4, label = paste("Correlation = ", Accuracy)), color = "black", size = 3) +
  scale_color_continuous(name = "Price")

# value
p_val <- ggplot(filtered_data, aes(x = review_scores_value, y = review_scores_rating, color=price)) + 
  geom_jitter(size = 0.4, alpha = 0.2, na.rm = TRUE) +
  geom_text(data = correlations, y = 0.2, aes(x = 4, label = paste("Correlation = ", Accuracy)), color = "black", size = 3) +
  scale_color_continuous(name = "Price")

# combine graphs
#plot_grid <- (p_acc | p_clean | p_check) /
#             (p_com | p_loc | p_val)

#plot_grid <- ggsave("balanced_composite_plot.png", plot_grid, width = 12, height = 8, units = "in", dpi = 300)
#plot_grid

score_price <- ggplot(filtered_data, aes(x = price, y = review_scores_rating, size = number_of_reviews)) +
  geom_point(alpha = 0.5) +
  labs(x = "Price", y = "Review Score", size = "Number of Reviews", title = "Review Score vs. Price")
score_price
...
```
Now basically cleaned the data, try to work on the prediction model.Firstly we will try the linear regression, then random forest model
```{r}
